...
MyBatis的动态SQL的使用比较简单：就是使用**简化的标签**。
说实话我觉得和 JSP很像，但是jsp太重量级了，而这个就是为了简化的，所以用起来方便；
...
我自己的一个小发现：在使用**类似MybatisX**之类的插件的时候，它根据你接口生成的**SQL映射的标签名是根据你的方法名来的**：所以接口方法**命名一定要规范**，在使用 Spring 相关的框架的时候一个重要的原则就是**约定大于配置**本质还是一种面向接口编程的思想。
当然如果匹配不上它会直接**询问你要用的是哪种SQL映射**

MyBatis的事务：
它**默认开启事务，并且关闭自动提交**。要么手动提交要么开启自动提交（实例化session的时候加一个参数）。

**未确认**：MyBatis如果在程序结束之前没有提交事务，DDL的修改会自动回滚，换句话说所有的DDL语句都会失效；

主键的问题：
进行添加业务的时候不会输入主键，因此 主键（SQL层面）不能绑定到对应的 Java对象上去，所以如果你直接返回 主键字段会是一个null；MyBatis提供了解决方案：
SQL映射的时候添加两个属性：`useGeneratedKeys` 和 `keyProperty`，分别是是否开启 主键绑定的功能以及这个主键是哪个字段。

**使用注解开发的话**在对应的dao层的接口上写`@options`注解，再把这两个属性添加进去

### 修改功能
修改的本质先查询再添加
修改本质和前面的内容是一样的

**为了防止忘记再写一遍**mybatis的SQL映射里面类似 where和set之类的全部替换成**对应的标签**，防止混乱的sql语句拼接。

一个update语句： update 《table_name》 set 《fields》 where 《condition》
